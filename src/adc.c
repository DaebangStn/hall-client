...
esp_err_t check_echoed_tx(int8_t adc_idx, spi_transaction_t t);
...
void post_spi_read(uint8_t idx_adc, spi_transaction_t* tx, float* v) {
    ...
    for(int i=2; i<6; i++) {
        if(check_echoed_tx((int8_t)idx_adc, tx[i]) != ESP_OK) {
            ESP_LOGE(TAG, "[ADC %d] returned register values do not match sent values", idx_adc);
            ESP_LOGW(TAG, "sent:");
    ...
    for (uint8_t i=0; i<6; i++) {
        ...
        while (gpio_get_level(adc_pins.drdy[idx_adc]) == 1) {
            if (esp_timer_get_time() - now > ADC_DRDY_TIMEOUT_US) {
                ESP_LOGE(TAG, "ADC[%d] timeout waiting for drdy", idx_adc);
                break;
            }
        }
...
esp_err_t check_echoed_tx(int8_t idx_adc, spi_transaction_t t) {
    uint8_t* out = (uint8_t*)t.rx_data;
    uint8_t* in = (uint8_t*)t.tx_data;
    if (out[2] != in[0] || out[3] != in[1]) {
        ESP_LOGE(TAG, "[ADC %d] config echoed incorrectly", idx_adc);
